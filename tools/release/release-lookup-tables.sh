...
echo "---------------------"
echo "Release Lookup Tables"
echo "---------------------"
echo "Cleanup WORKING_DIR"
rm -rf $WORKING_DIR/oersi-lookup-tables
cd $WORKING_DIR
git clone git@gitlab.com:oersi/oersi-lookup-tables.git -b main
if [ $? -ne 0 ] ; then
  echo "Cloning failed."
  exit 1
fi
cd $WORKING_DIR/oersi-lookup-tables
