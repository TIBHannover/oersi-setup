...
GITLAB_PROJECT_ID_LOOKUPTABLES="81536476"
...
SIDRE_IMPORTSCRIPTS_COMMONS_RELEASE_ARTIFACT_URL=$(get_release_artifact_url $GITLAB_PROJECT_ID_SIDRE_IMPORTSCRIPTS_COMMONS "import-scripts-commons.zip" $SIDRE_VERSION)
...
release_module "release-lookup-tables.sh" $RELEASE_LOOKUPTABLES
...
for run in {1..30}; do
  ...
  LOOKUPTABLES_RELEASE_ARTIFACT_URL=$(get_release_artifact_url $GITLAB_PROJECT_ID_LOOKUPTABLES "lookup-tables.zip")

  date +"%Y-%m-%d %H:%M:%S package urls: etl $ETL_RELEASE_ARTIFACT_URL, import-scripts $IMPORTSCRIPTS_RELEASE_ARTIFACT_URL, marc-transformation $MARC_TRANSFORMATION_RELEASE_ARTIFACT_URL, schema $SCHEMA_RELEASE_ARTIFACT_URL, lookup-tables $LOOKUPTABLES_RELEASE_ARTIFACT_URL"
  if [ -z "$ETL_RELEASE_ARTIFACT_URL" ] || [ -z "$IMPORTSCRIPTS_RELEASE_ARTIFACT_URL" ] || [ -z "$MARC_TRANSFORMATION_RELEASE_ARTIFACT_URL" ] || [ -z "$SCHEMA_RELEASE_ARTIFACT_URL" ] || [ -z "$LOOKUPTABLES_RELEASE_ARTIFACT_URL" ] ; then
...
if [ -z "$LOOKUPTABLES_RELEASE_ARTIFACT_URL" ] ; then
  echo "Missing lookup-tables artifact for version $RELEASE_VERSION"
fi
if [ -z "$ETL_RELEASE_ARTIFACT_URL" ] || [ -z "$MARC_TRANSFORMATION_RELEASE_ARTIFACT_URL" ] || [ -z "$IMPORTSCRIPTS_RELEASE_ARTIFACT_URL" ] || [ -z "$SCHEMA_RELEASE_ARTIFACT_URL" ] || [ -z "$LOOKUPTABLES_RELEASE_ARTIFACT_URL" ] ; then
  exit 1
fi
...
echo $LOOKUPTABLES_RELEASE_ARTIFACT_URL

date +"%Y-%m-%d %H:%M:%S Release setup"
# RELEASE SETUP -> no need to wait for artifact, as setup release does not produce artifacts
release_module "release-setup.sh" true $ETL_RELEASE_ARTIFACT_URL $IMPORTSCRIPTS_RELEASE_ARTIFACT_URL $SCHEMA_RELEASE_ARTIFACT_URL $LOOKUPTABLES_RELEASE_ARTIFACT_URL $SIDRE_SETUP_RELEASE_ARTIFACT_URL
...
create_gitlab_release_entry $GITLAB_PROJECT_ID_LOOKUPTABLES $RELEASE_LOOKUPTABLES $LOOKUPTABLES_RELEASE_ARTIFACT_URL
