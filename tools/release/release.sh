...
if [ "$WAIT_FOR_RELEASE_ARTIFACTS" = true ] ; then
  ...
  do
    ...
    ETL_PACKAGE_ID=$(curl -s "${GITLAB_FRONTEND_API}/packages" | jq ".[] | select(.version == \"$FRONTEND_RELEASE_VERSION\") | .id")
    FRONTEND_PACKAGE_ID=$(curl -s "${GITLAB_ETL_API}/packages" | jq ".[] | select(.version == \"$ETL_RELEASE_VERSION\") | .id")
    date +"%Y-%m-%d %H:%M:%S package ids: backend $BACKEND_PACKAGE_ID, etl $ETL_PACKAGE_ID, frontend $FRONTEND_PACKAGE_ID"
    if [ -z "$BACKEND_PACKAGE_ID" ] || [ -z "$ETL_PACKAGE_ID" ] || [ -z "$FRONTEND_PACKAGE_ID" ]; then
...
if [ -z "$BACKEND_PACKAGE_FILE_ID" ] || [ -z "$FRONTEND_PACKAGE_FILE_ID" ] || [ -z "$ETL_PACKAGE_FILE_ID" ] ; then
  exit 1
fi
...
$SCRIPT_DIR/release-setup.sh $RELEASE_VERSION $NEXT_VERSION $RELEASE_ISSUE_URL $PUSH_TO_ORIGIN $WORKING_DIR $BACKEND_RELEASE_ARTIFACT_URL $ETL_RELEASE_ARTIFACT_URL $FRONTEND_RELEASE_ARTIFACT_URL
curl --header 'Content-Type: application/json' --header "PRIVATE-TOKEN: $GITLAB_ACCESS_TOKEN" \
