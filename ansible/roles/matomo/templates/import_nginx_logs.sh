...
find /var/log/nginx -type f -name "access.log*[0-9]" -newermt `date --date="yesterday" +"%Y-%m-%d"` ! -newermt `date +"%Y-%m-%d"` | while read logfile ; do
  ...
  python {{ matomo_inst_dir }}/matomo/misc/log-analytics/import_logs.py --url=http://localhost:{{ matomo_port }}/matomo --idsite={{ matomo_init_website.idsite }} --log-format-regex='([\w*.:-]+)\s+\S+\s+(?P<userid>\S+)\s+\[(?P<date>.*?)\s+(?P<timezone>.*?)\]\s+"(?P<method>\S+)\s+(?P<path>.*?)\s+\S+"\s+(?P<status>\d+)\s+(?P<length>\S+)\s+"(?P<referrer>.*?)"\s+"(?P<user_agent>.*?)"\s+"(?P<ip>[\w*.:-]+)"' $logfile
done
